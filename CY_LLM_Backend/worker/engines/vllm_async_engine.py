...
def _ensure_vllm_async_imported():
    ...
    if not _vllm_async_imported:
        try:
            ...
            raise ImportError(
                f"vLLM 异步模块未安装。请运行: pip install vllm\n原始错误: {e}"
            ) from e
...
class VllmAsyncEngine(BaseEngine):
    ...
    def __init__(
        ...
        gpu_memory_utilization: float = 0.75,  # 修复: 与VllmCudaEngine保持一致，避免OOM
        ...
        allow_auto_tuning: bool = True,  # 新增: 自动调整参数避免OOM
        **kwargs,
    ) -> None:
        """
        初始化 vLLM 异步引擎。
        ...
        Args:
            ...
            gpu_memory_utilization: GPU 显存使用率上限（默认0.75，推荐0.70-0.85）
        ...
        # 验证 gpu_memory_utilization 安全性（与VllmCudaEngine保持一致）
    ...
    async def load_model_async(
        self, model_path: str, adapter_path: Optional[str] = None, **kwargs
    ) -> None:
        """
        异步加载模型到 GPU。
    ...
    def infer(
        ...
        Returns:
            Generator[str, None, None]: 流式生成的文本块
    ...
    async def infer_batch_async(self, prompts: List[str], **kwargs) -> List[str]:
        """
        异步批量推理。
