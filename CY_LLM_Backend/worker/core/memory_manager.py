...
class GPUMemoryManager:
    ...
    def __new__(cls, threshold: Optional[float] = None, **kwargs):
        # 测试模式下不使用单例，每次创建新实例
    ...
    def register_model(self, model_id: str, engine: Optional['BaseEngine'] = None, size_gb: float = 0.0):
        """注册一个已加载的模型引擎
    ...
    def release_lock(self, model_id: str):
        """释放模型锁 (兼容旧 API)"""
        if model_id in self.model_locks:
            try:
